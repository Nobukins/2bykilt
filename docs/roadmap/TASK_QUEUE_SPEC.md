...
`TASK_QUEUE.yml` ファイルは、`ISSUE_DEPENDENCIES.yml` から自動生成される機械可読のタスクキューです。依存関係を考慮した優先順位付けられたIssueビューを提供し、自動化とワークフロー管理のためにステータス別に整理されています。

**⚠️ 手動編集禁止**: このファイルは `scripts/generate_task_queue.py` によって自動生成されます。変更するには `docs/roadmap/ISSUE_DEPENDENCIES.yml` を更新し、ジェネレーターを再実行してください。
...
- Issue has a primary PR defined in dependencies (simplified status; full PR API integration pending)
...
3. **`longest_distance`**: Topological distance from leaf nodes (longer paths first) - Note: This is used internally for ordering but not included in the output schema
...
This enrichment improves status classification accuracy but is optional. Note: Primary PR status enrichment is not currently implemented.

## Usage Patterns

### Automation Workflow

The TASK_QUEUE.yml is automatically maintained through GitHub Actions workflow (`/.github/workflows/dependency-pipeline.yml`).

#### Workflow Triggers

- **Pull Request**: When `docs/roadmap/ISSUE_DEPENDENCIES.yml` changes on PR to default branch
- **Push**: When `docs/roadmap/ISSUE_DEPENDENCIES.yml` is pushed to default branch
- **Schedule**: Daily at 3 AM UTC for maintenance updates

#### Workflow Jobs

1. **validate-deps**: Validates ISSUE_DEPENDENCIES.yml structure and dependencies
2. **regenerate-and-commit**:
   - Regenerates TASK_QUEUE.yml, DEPENDENCY_GRAPH.md, TASK_DASHBOARD.md
   - Validates generated artifacts
   - Auto-commits changes with message "docs: auto-update dependency artifacts"
   - Skips commit on PR context (detached HEAD)

#### Dependencies

- **Input**: `docs/roadmap/ISSUE_DEPENDENCIES.yml` (canonical source)
- **Outputs**:
  - `docs/roadmap/TASK_QUEUE.yml` (this file)
  - `docs/roadmap/DEPENDENCY_GRAPH.md` (Mermaid diagram)
  - `docs/roadmap/TASK_DASHBOARD.md` (human-readable summary)
- **Environment**: Requires `GITHUB_TOKEN` for API enrichment (optional)

#### Integration Points

- **ROADMAP.md**: References TASK_QUEUE.yml for progress tracking
- **Issue Management**: Provides machine-readable task queue for automation
- **CI/CD**: Validates dependency integrity before merges### Manual Usage
