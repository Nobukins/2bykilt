...
graph TD
    A[ユーザー] --> B(Gradio UI);
    B -- タスク入力: `search-linkedin query=AI` --> C{bykilt.py バックエンド};
...
*   **トリガー入力 (Trigger Input):**
    *   [GradioウェブUI](01_gradioウェブui_.md)の「タスク説明」欄に入力する、特定のスクリプトを実行するための「合言葉」です。`llms.txt`で定義された形式に従います。（例: `search-linkedin query="AIエンジニア"`）（図の `B` -> `C`）
*   **LLMバイパス (LLM Bypass):**
