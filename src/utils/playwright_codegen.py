...
def run_edge_codegen(url):
    ...
    try:
        ...
        user_data_dir = browser_paths["edge_user_data"]
        
        if not user_data_dir or not os.path.exists(user_data_dir):
            return False, "Edgeユーザープロファイルディレクトリが見つかりません"
        
        # コマンドを構築 - 正しいオプション構成に修正
        cmd = [
            "playwright", "codegen",
            url,
            "--target", "python",
        ...
        # 環境変数を設定してEdgeにユーザーデータディレクトリを使用させる
        env = os.environ.copy()
        # Python版Playwrightでの正しい環境変数
