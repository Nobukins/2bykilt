...
class TestPlaywrightEngineLaunch:
    ...
    async def test_launch_default_context(self, engine):
        """デフォルト設定でブラウザを起動"""
    ...
    async def test_launch_with_trace(self, engine):
        """トレース有効化でブラウザを起動"""
        with patch("src.browser.engine.playwright_engine.async_playwright") as mock_pw:
            mock_pw_instance = AsyncMock()
            mock_pw.return_value.start = AsyncMock(return_value=mock_pw_instance)
...
class TestPlaywrightEngineActions:
    ...
    @pytest.fixture
    async def launched_engine(self, engine):
        """起動済みエンジン（モック）"""
    ...
    async def test_navigate(self, launched_engine):
        """ナビゲーション実行"""
    ...
    async def test_dispatch_click(self, launched_engine):
        """クリックアクション"""
    ...
    async def test_dispatch_fill(self, launched_engine):
        """フォーム入力アクション"""
    ...
    async def test_dispatch_screenshot(self, launched_engine):
        """スクリーンショットアクション"""
        launched_engine._page.screenshot = AsyncMock()
...
class TestPlaywrightEngineShutdown:
    ...
    async def test_shutdown_with_capture(self, engine):
        """最終状態キャプチャ付きシャットダウン"""
        engine._page = AsyncMock()
        engine._context = AsyncMock()
        engine._browser = AsyncMock()
